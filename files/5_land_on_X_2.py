#!/usr/bin/env python3
"""
Script 11: LAND ON X — Latch & Commit + LIVE BROWSER FEED
=============================================================
The problem with visual-servoing all the way to ground:
    At low altitude, the X overflows the camera frame. Any CV method
    (skeleton, Hough, centroid) finds the center of a VISIBLE ARM,
    not the center of the X. The drone chases that phantom center
    forever → infinite hover loop.

Solution — "Latch & Commit":
    Phase 1 (YOLO, above LATCH_ALT):
        Standard YOLO detection → center over X → descend in steps.
        This is proven and works well when the full X is visible.

    Phase 2 (LATCH, at LATCH_ALT):
        Require multiple consecutive "centered" YOLO frames.
        Record the drone's GPS coordinates at each centered frame.
        Average them → this IS the X's GPS position.
        Lock it. Never update it.

    Phase 3 (COMMIT DESCENT, below LATCH_ALT):
        Stop all vision corrections.
        Command the Pixhawk to HOLD the latched GPS position.
        Descend vertically at a steady rate.
        Pixhawk's EKF (GPS + IMU + baro) keeps position ±0.5-1m.

    Phase 4 (LAND):
        Switch to LAND mode. Pixhawk handles touchdown.

Error budget:
    Centering error at latch:  ~0.1-0.2m (25px deadzone at 3m)
...
State machine:
    ...
    LATCH_CENTERING → LATCH_CONFIRM → COMMIT_DESCENT → LAND

Usage:
    python3 11_land_on_x_latch.py --alt 5
    python3 11_land_on_x_latch.py --alt 5 --feed-port 5000
    python3 11_land_on_x_latch.py --dry-run       # ground test, no flight
    python3 11_land_on_x_latch.py --sitl
...
Terminal 2: python3 11_land_on_x_latch.py
...
LATCH_DEADZONE   = 25        # px — must be this centered to latch
LATCH_FRAMES     = 8         # consecutive centered frames required
LATCH_MAX_SPREAD = 1.5       # meters — max GPS spread during latch
                              # (reject if GPS is jumping around)
...
<body>
    ...
    <p class="info">Status: <span>STREAMING</span> | Latch & Commit landing</p>
...
def update_feed_frame(overlay_frame):
    """Push the latest overlay frame to the browser stream."""
    global latest_frame
...
def start_feed_server(port):
    """Start Flask in a daemon thread — won't block the flight loop."""
    import subprocess
    try:
...
def send_position_with_descent(fc, lat, lon, alt):
    ...
    The Pixhawk's position controller handles the hold — much better
    than velocity commands which time out and drift.
    """
...
def draw_overlay(frame, state, det, cur_alt, fc,
    ...
    latch_info: dict with latch state details (count, required, etc.)
    commit_info: dict with commit state details (latched pos, drift, etc.)
    """
    h, w = frame.shape[:2]
    cx, cy = w // 2, h // 2
    ...
    if state in ("LATCH_CENTERING", "LATCH_CONFIRM"):
        dz = LATCH_DEADZONE
    ...
    # ── Latch progress bar ───────────────────────────────────
    if latch_info:
        count = latch_info.get('count', 0)
        required = latch_info.get('required', LATCH_FRAMES)
        bar_x, bar_y = 10, h - 70
        bar_w, bar_h = 200, 20
        cv2.rectangle(frame, (bar_x, bar_y),
                      (bar_x + bar_w, bar_y + bar_h), COLOR_LATCH, 1)
        fill_w = int(bar_w * count / required)
        cv2.rectangle(frame, (bar_x, bar_y),
                      (bar_x + fill_w, bar_y + bar_h), COLOR_LATCH, -1)
        cv2.putText(frame, f"LATCH: {count}/{required}",
    ...
    if commit_info:
        ...
        # Big magenta cross — GPS hold indicator
    ...
    elif state in ("LATCH_CENTERING", "LATCH_CONFIRM"):
        phase = "LATCHING"
...
def process_frame(frame, state, det, cur_alt, fc, vw,
                  centered=False, latch_info=None, commit_info=None):
    """Draw overlay, write to video file, push to live browser feed.
    Returns 1 if frame was written, 0 otherwise."""
    if frame is None:
        return 0
...
# ===========================================================================
# CSV DATA LOGGER (for post-flight analysis)
# ===========================================================================

def create_csv_log(prefix="flight_data"):
    """Create a CSV file for per-frame telemetry logging."""
    ...
    f.write("timestamp,state,alt_m,lat,lon,gps_fix,satellites,"
            ...
            "latch_count,latch_lat,latch_lon,"
            "drift_from_latch_m,"
            "frame_num,notes\n")
    return fname, f
...
def csv_row(f, state, fc, cur_alt, det=None,
            ...
            vx=0, vy=0, vz=0,
            latch_count=0, latch_lat=0, latch_lon=0,
    ...
    f.write(f"{ts},{state},{cur_alt:.2f},"
            ...
            f"{vx:.4f},{vy:.4f},{vz:.4f},"
            f"{latch_count},{latch_lat:.8f},{latch_lon:.8f},"
...
def main(args):
    if not args.dry_run and not args.sitl:
        if not confirm("11_land_on_x_latch.py — LATCH & COMMIT + LIVE FEED",
                       f"Takeoff {args.alt}m → Find X (YOLO) → Center → "
                       f"Descend to {LATCH_ALT}m →\n"
                       f"  Latch GPS ({LATCH_FRAMES} frames) → "
                       f"Commit descent (GPS hold) → LAND\n"
    ...
    if cap:
        ...
        video_path_tmp = f"landing_latch_{ts}_tmp.mp4"
        video_path = f"landing_latch_{ts}.mp4"
    ...
    # ── Logs ─────────────────────────────────────────────────
    log_fname, log_f = create_log("landing_latch")
    csv_fname, csv_f = create_csv_log("landing_latch_data")

    log(log_f, "=" * 60)
    log(log_f, "LATCH & COMMIT PRECISION LANDING")
    log(log_f, "=" * 60)
    log(log_f, f"Takeoff alt:     {args.alt}m")
    log(log_f, f"Latch alt:       {LATCH_ALT}m")
    log(log_f, f"Latch frames:    {LATCH_FRAMES}")
    ...
    with SafeFlight(fc, camera=cap, video_writer=vw) as sf:
        ...
        # ── State variables ──────────────────────────────────
        state = "TAKEOFF"
        last_x = 0                 # last time YOLO saw the X
        search_t0 = 0              # when search started
        descend_tgt = 0            # target alt for current descent step

        # Latch state
        latch_count = 0            # consecutive centered frames
        latch_gps_readings = []    # list of (lat, lon) during latch
        latch_lat = 0.0            # final latched latitude
        latch_lon = 0.0            # final latched longitude
        latch_locked = False       # True once GPS is latched

        # Commit state
        commit_t0 = 0              # when commit descent started
        commit_last_cmd = 0        # last time we sent position command
        commit_target_alt = 0      # current target alt for descent
        ...
        while state not in ("DONE", "ABORT"):
            ...
            if cap:
                ...
                if ret and state not in ("COMMIT_DESCENT", "LAND", "LANDING"):
                    # Only run YOLO when we need it (not during commit)
                    det = detect_x(frame, model, args.conf, args.imgsz)
                elif ret:
                    pass  # frame read but no YOLO during commit
                else:
                    frame = None
            ...
            # ── ACQUIRE (YOLO centering) ─────────────────────
                ...
                if abs(dx_px) <= dz and abs(dy_px) <= dz:
                    ...
                    # Re-draw with centered flag for video + feed
                    if frame is not None:
            ...
            # ── LATCH_CENTERING (final tight centering at latch alt) ─
            elif state == "LATCH_CENTERING":
                if det is None:
                    lost = time.time() - last_x
                    if lost > LOST_TIMEOUT:
                        log(log_f, f"LOST during latch centering {lost:.0f}s → RTL")
                        csv_row(csv_f, state, fc, cur_alt,
                    ...
                    csv_row(csv_f, state, fc, cur_alt,
                            ...
                            notes=f"lost_{lost:.1f}s_latch_reset")
                    print(f"\r  [LATCH_CENTER] Lost X ({lost:.1f}s) "
                          f"— reset count   ", end="", flush=True)
                    time.sleep(VEL_RATE)
                    continue
                ...
                if abs(dx_px) <= LATCH_DEADZONE and abs(dy_px) <= LATCH_DEADZONE:
                    # Centered frame — count it and record GPS
                    latch_count += 1
                    latch_gps_readings.append((fc.lat, fc.lon))

                    log(log_f, f"LATCH frame {latch_count}/{LATCH_FRAMES} "
                               f"({dx_px:+d},{dy_px:+d})px = {dist_m:.3f}m | "
                    ...
                    csv_row(csv_f, state, fc, cur_alt, det,
                            ...
                            frame_num=frame_count,
                            notes=f"latch_frame_{latch_count}")

                    if not args.dry_run:
                        fc.stop()  # hold position

                    # Re-draw with centered flag + latch bar
                    if frame is not None:
                        li = {'count': latch_count, 'required': LATCH_FRAMES}
                        frame_count += process_frame(frame, state, det,
                                                      cur_alt, fc, vw,
                                                      centered=True,
                                                      latch_info=li)

                    print(f"\r  [LATCH] {'●' * latch_count}"
                          f"{'○' * (LATCH_FRAMES - latch_count)} "
                          f"{latch_count}/{LATCH_FRAMES} | "
                          f"({dx_px:+d},{dy_px:+d})px | "
                          f"GPS=({fc.lat:.7f},{fc.lon:.7f})   ",
                          end="", flush=True)

                    if latch_count >= LATCH_FRAMES:
                        state = "LATCH_CONFIRM"
                        continue
                    time.sleep(VEL_RATE)
                    continue
                else:
                    # Not centered — reset count and correct
                    if latch_count > 0:
                        log(log_f, f"LATCH RESET (was {latch_count}) — "
                                   f"drifted to ({dx_px:+d},{dy_px:+d})px")
                    latch_count = 0
                    latch_gps_readings = []
                    ...
                    csv_row(csv_f, state, fc, cur_alt, det,
                            dx_px, dy_px, m_fwd, m_right, dist_m,
                            vx, vy, 0, latch_count=0,
                            frame_num=frame_count, notes="centering")

                    print(f"\r  [LATCH_CENTER] centering "
                          f"({dx_px:+d},{dy_px:+d})px {dist_m:.2f}m | "
                          f"v=({vx:.2f},{vy:.2f})   ",
                          end="", flush=True)
                    time.sleep(VEL_RATE)

            # ── LATCH_CONFIRM (validate and lock GPS) ────────
            elif state == "LATCH_CONFIRM":
                spread = gps_spread(latch_gps_readings)
                avg_lat = sum(r[0] for r in latch_gps_readings) / len(latch_gps_readings)
                avg_lon = sum(r[1] for r in latch_gps_readings) / len(latch_gps_readings)

                log(log_f, "")
                log(log_f, "=" * 55)
                log(log_f, "  GPS LATCH VALIDATION")
                log(log_f, "=" * 55)
                log(log_f, f"  Readings:  {len(latch_gps_readings)}")
                log(log_f, f"  GPS spread: {spread:.3f}m "
                           f"(max allowed: {LATCH_MAX_SPREAD}m)")
                log(log_f, f"  Average:   ({avg_lat:.8f}, {avg_lon:.8f})")
                log(log_f, f"  Altitude:  {cur_alt:.1f}m")
                log(log_f, f"  Satellites: {fc.satellites}  Fix: {fc.gps_fix}")
                for i, (lat, lon) in enumerate(latch_gps_readings):
                    d = haversine(lat, lon, avg_lat, avg_lon)
                    log(log_f, f"    [{i+1}] ({lat:.8f}, {lon:.8f}) — "
                               f"{d:.3f}m from avg")
                log(log_f, "=" * 55)

                if spread > LATCH_MAX_SPREAD:
                    log(log_f, f"  ✗ REJECTED — GPS spread {spread:.3f}m > "
                               f"{LATCH_MAX_SPREAD}m")
                    log(log_f, f"  Retrying latch centering...")
                    log(log_f, "=" * 55)
                    csv_row(csv_f, state, fc, cur_alt,
                            latch_count=latch_count,
                            latch_lat=avg_lat, latch_lon=avg_lon,
                            frame_num=frame_count,
                            notes=f"LATCH_REJECTED_spread={spread:.3f}")
                    latch_count = 0
                    latch_gps_readings = []
                    state = "LATCH_CENTERING"
                    continue
                ...
                log(log_f, f"  Spread:   {spread:.3f}m (good)")
                log(log_f, f"  This position will be held during descent.")
                ...
                csv_row(csv_f, state, fc, cur_alt,
                        ...
                        frame_num=frame_count,
                        notes=f"LATCHED_spread={spread:.3f}")

                print(f"\n\n  ★ GPS LATCHED at ({latch_lat:.8f}, {latch_lon:.8f})")
                print(f"    Spread: {spread:.3f}m | Alt: {cur_alt:.1f}m")
            ...
            # ══════════════════════════════════════════════════
                ...
                # Log if YOLO still sees X (just for data — NOT used for control)
                if det:
                    dx_px = det['cx'] - FRAME_W // 2
                    dy_px = det['cy'] - FRAME_H // 2
                    m_fwd, m_right = pixels_to_meters(dx_px, dy_px, cur_alt)
            ...
            # ── LAND ─────────────────────────────────────────
                ...
                if not args.dry_run:
                    ...
                    # Keep recording + streaming during landing
                    land_t0 = time.time()
                    while fc.armed and (time.time() - land_t0 < 30):
                        ...
                        # CSV log every ~0.5s during landing
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Latch & Commit Precision X Landing (with live feed)")
