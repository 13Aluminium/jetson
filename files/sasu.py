#!/usr/bin/env python3
"""
grandmother.py — Dual Mission: Ball Drop + Waypoint Navigation
================================================================
The "grandmother script" chains TWO missions in a single flight:

  MISSION 1 — BALL DROP (from mother.py):
  ...
  MISSION 2 — WAYPOINT NAVIGATION (after drop):
    8. WAYPOINT_CLIMB — climb to 52 ft (15.85m)
    9. WAYPOINT_NAV — fly through 3 hardcoded waypoints at 5 m/s:
         WP4 (35.050101, -118.150687)  →
         WP1 (35.049875, -118.150127)  →
         WP7 (35.048843, -118.151194)
   10. RETURNING — RTL after final waypoint
...
State Machine:
    ...
    ACQUIRING → DESCENDING → DROP_ALIGNMENT → DROPPING →
    WAYPOINT_CLIMB → WAYPOINT_NAV → RETURNING → DONE

Usage:
    python3 grandmother.py --lat 35.049803 --lon -118.150986

    python3 grandmother.py --lat 35.049803 --lon -118.150986 --speed 0.5 \\
                           --alt 5 --max-alt 15 --drop-alt 3

    python3 grandmother.py --lat 35.049803 --lon -118.150986 --dry-run

Terminal 1: mavproxy.py --master=/dev/ttyACM0 --baudrate=115200 \\
            --out=udp:127.0.0.1:14551
Terminal 2: python3 grandmother.py --lat <lat> --lon <lon>
...
# ── MISSION 2: Waypoint Navigation (HARDCODED) ───────────────
# After ball drop, climb to this altitude then fly waypoints
WP_NAV_ALT    = 15.85     # 52 feet in meters (52 × 0.3048)
WP_NAV_SPEED  = 5.0       # m/s cruise speed for waypoint nav
WP_NAV_ARRIVE = 2.0       # meters — "arrived" radius for waypoints
WP_NAV_TIMEOUT = 120      # seconds max per waypoint leg

# The 3 waypoints in flight order (WP4 → WP1 → WP7)
WAYPOINTS = [
...
FEED_HTML = """<!DOCTYPE html><html><head><title>Grandmother Mission</title>
...
</style></head><body><h1>Grandmother Mission — Live Feed</h1>
...
def draw_overlay(frame, state, det, cur_alt, fc,
    ...
    phase_colors = {
        ...
        "WAYPOINT_CLIMB": COLOR_NAV, "WAYPOINT_NAV": COLOR_NAV,
        "CLIMBING_RTL": COLOR_DROPPED,
    }
...
def main(args):
    ...
    if not args.dry_run and not args.sitl:
        dist_est = "unknown"
        desc = (
            f"DUAL MISSION: BALL DROP + WAYPOINT NAV\n"
            ...
            f"  Mission 1:   Takeoff → Fly to GPS → Search X → Drop ball\n"
            f"  Mission 2:   Climb 52ft → WP4 → WP1 → WP7 @ {WP_NAV_SPEED}m/s → RTL"
        )
        if not confirm("grandmother.py — DUAL MISSION", desc):
            return
    ...
    if cap:
        ...
        video_path_tmp = f"grandmother_mission_{ts_str}_tmp.mp4"
        video_path = f"grandmother_mission_{ts_str}.mp4"
    ...
    # ── Logs ──────────────────────────────────────────────────
    log_fname, log_f = create_log("grandmother_mission")
    csv_fname, csv_f = create_csv_log("grandmother_data")

    log(log_f, "=" * 60)
    log(log_f, "GRANDMOTHER MISSION — BALL DROP + WAYPOINT NAV")
    log(log_f, "=" * 60)
    ...
    log(log_f, "")
    log(log_f, "MISSION 2 — WAYPOINT NAVIGATION:")
    log(log_f, f"  Nav altitude:  {WP_NAV_ALT:.2f}m (52 ft)")
    log(log_f, f"  Nav speed:     {WP_NAV_SPEED} m/s")
    for wp in WAYPOINTS:
    ...
    with SafeFlight(fc, camera=cap, video_writer=vw) as sf:
        ...
        # Waypoint navigation state (Mission 2)
        wp_index = 0            # index into WAYPOINTS list
        ...
        while state not in ("DONE", "ABORT"):
            ...
            # ══════════════════════════════════════════════════
                ...
                # Close claw after drop, release RC override
                log(log_f, "Payload dropped — closing claw for waypoint nav")
                if not args.dry_run:
                    set_claw(fc, CLAW_CLOSE_PWM, log_f)
                    time.sleep(0.5)
                    release_rc_override(fc)
                ...
                state = "WAYPOINT_CLIMB"
                continue

            # ══════════════════════════════════════════════════
            # WAYPOINT CLIMB — climb to 52 ft for waypoint nav
            # ══════════════════════════════════════════════════
            elif state == "WAYPOINT_CLIMB":
                log(log_f, "")
                log(log_f, "=" * 60)
                log(log_f, "  ★ MISSION 2: WAYPOINT NAVIGATION  ★")
                log(log_f, "=" * 60)
                log(log_f, f"Climbing to {WP_NAV_ALT:.1f}m (52 ft) for waypoint nav")

                print(f"\n{'='*60}")
                print(f"  ★ MISSION 2: WAYPOINT NAVIGATION")
                print(f"  Climbing to {WP_NAV_ALT:.1f}m (52 ft)...")
                print(f"{'='*60}\n")
                ...
                if not args.dry_run:
                    ...
                    while True:
                        fc.poll()
                        if fc.alt >= WP_NAV_ALT - 0.5:
                            break
                        if time.time() - t0 > 30:
                            log(log_f, "WP climb timeout 30s — continuing at current alt")
                            break
                        fc.velocity_ned(0, 0, -0.8)  # climb at 0.8 m/s

                        if cap:
                            ret, frm = cap.read()
                            if ret:
                                frame_count[0] += process_frame(
                                    frm, "WAYPOINT_CLIMB", None, fc.alt, fc, vw,
                                    dropped=True,
                                    mission_elapsed=time.time() - mission_t0,
                                    extra_info=f"Climbing to {WP_NAV_ALT:.0f}m")
                        time.sleep(VEL_RATE)
                ...
                fc.poll()
                log(log_f, f"At {fc.alt:.1f}m — starting waypoint navigation")
                log(log_f, f"Speed: {WP_NAV_SPEED} m/s | Waypoints: "
                           f"WP{WAYPOINTS[0]['id']} → WP{WAYPOINTS[1]['id']} → WP{WAYPOINTS[2]['id']}")

                # Set waypoint nav speed
                if not args.dry_run:
                    set_speed(fc, WP_NAV_SPEED)

                wp_index = 0  # start with first waypoint
                state = "WAYPOINT_NAV"
                continue

            # ══════════════════════════════════════════════════
            # WAYPOINT NAV — fly WP4 → WP1 → WP7 at 5 m/s
            # ══════════════════════════════════════════════════
            elif state == "WAYPOINT_NAV":
                ...
                fc.poll()
                wp_dist = haversine(fc.lat, fc.lon, wp_lat, wp_lon)
                wp_brng = bearing(fc.lat, fc.lon, wp_lat, wp_lon)
                wp_compass = bearing_to_compass(wp_brng)

                log(log_f, f"WAYPOINT NAV → WP{wp_num} ({wp_lat:.6f}, {wp_lon:.6f}) "
                           f"dist={wp_dist:.1f}m bearing={wp_brng:.0f}° ({wp_compass})")
                print(f"\n  [WP NAV] Flying to WP{wp_num} — "
                      f"{wp_dist:.1f}m {wp_compass} at {WP_NAV_SPEED} m/s")

                csv_row(csv_f, state, fc, fc.alt,
                        mission_elapsed=time.time() - mission_t0,
                        frame_num=frame_count[0],
                        notes=f"heading_to_WP{wp_num}")
                ...
                while True:
                    ...
                    if now - last_cmd >= 2.0:
                        if not args.dry_run:
                            send_goto(fc, wp_lat, wp_lon, WP_NAV_ALT)
                        last_cmd = now

                    remaining = haversine(fc.lat, fc.lon, wp_lat, wp_lon)
                    pct = max(0, (1 - remaining / wp_dist)) * 100 if wp_dist > 0 else 100

                    # Record video during waypoint flight
                    if cap:
                        ret, frm = cap.read()
                        if ret:
                            frame_count[0] += process_frame(
                                frm, "WAYPOINT_NAV", None, fc.alt, fc, vw,
                                dropped=True,
                                mission_elapsed=mission_elapsed,
                                extra_info=f"WP{wp_num} rem={remaining:.1f}m "
                                           f"({wp_index+1}/{len(WAYPOINTS)})")

                    csv_row(csv_f, state, fc, fc.alt,
                            mission_elapsed=mission_elapsed,
                            frame_num=frame_count[0],
                            notes=f"WP{wp_num}_remaining={remaining:.1f}m")

                    print(f"\r  [WP{wp_num}] {pct:5.1f}%  rem={remaining:.1f}m  "
                          f"alt={fc.alt:.1f}m  sats={fc.satellites}  "
                          f"batt={fc.battery_pct}%  T+{mission_elapsed:.0f}s   ",
                          end="", flush=True)

                    # Arrived at waypoint?
                    if remaining <= WP_NAV_ARRIVE:
                        print()
                        log(log_f, f"✓ ARRIVED at WP{wp_num} — {remaining:.2f}m from target")
                        csv_row(csv_f, state, fc, fc.alt,
                                mission_elapsed=mission_elapsed,
                                frame_num=frame_count[0],
                                notes=f"ARRIVED_WP{wp_num}")
                        break

                    # Timeout safety
                    if now - nav_t0 > WP_NAV_TIMEOUT:
                        print()
                        log(log_f, f"WP{wp_num} NAV TIMEOUT {WP_NAV_TIMEOUT}s — "
            ...
            # ══════════════════════════════════════════════════
                ...
                log(log_f, "Waypoint nav complete → RTL")

                if not args.dry_run:
                    # safe_rtl handles climbing to safe alt + commanding RTL
                    safe_rtl(fc, log_f, cap, vw, frame_count, mission_t0)
                ...
                log(log_f, "")
                log(log_f, "=" * 60)
                log(log_f, "  ★ ★ ★  DUAL MISSION COMPLETE  ★ ★ ★")
                log(log_f, "=" * 60)
                log(log_f, f"  Mission 1 — Ball Drop:")
                log(log_f, f"    Drop GPS:    ({drop_lat:.8f}, {drop_lon:.8f})")
                log(log_f, f"    Drop alt:    {drop_alt:.1f}m")
                log(log_f, f"  Mission 2 — Waypoint Nav:")
                log(log_f, f"    WP4 → WP1 → WP7 at {WP_NAV_SPEED} m/s")
                log(log_f, f"    Nav alt:     {WP_NAV_ALT:.1f}m (52 ft)")
                log(log_f, f"  Total time:    {mission_total:.1f}s")
                log(log_f, f"  Battery:       {fc.battery_pct}%")
                log(log_f, "=" * 60)

                print(f"\n\n{'='*60}")
                print(f"  ★ ★ ★  DUAL MISSION COMPLETE  ★ ★ ★")
                if drop_locked:
                    print(f"  Drop position: ({drop_lat:.8f}, {drop_lon:.8f})")
                print(f"  Waypoints:     WP4 → WP1 → WP7 ✓")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Grandmother Mission — Ball Drop + Waypoint Navigation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 grandmother.py --lat 35.049803 --lon -118.150986
  python3 grandmother.py --lat 35.049803 --lon -118.150986 --speed 0.5 --alt 5
  python3 grandmother.py --lat 35.049803 --lon -118.150986 --alt 8 --max-alt 20 --drop-alt 3
  python3 grandmother.py --lat 35.049803 --lon -118.150986 --dry-run
  python3 grandmother.py --lat 35.049803 --lon -118.150986 --sitl

Hardcoded waypoints (Mission 2):
  WP4: (35.050101, -118.150687)
  WP1: (35.049875, -118.150127)
  WP7: (35.048843, -118.151194)
  Altitude: 52 ft (15.85m) | Speed: 5 m/s
""")
