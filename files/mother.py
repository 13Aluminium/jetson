...
The "mother script" that chains everything together:
    ...
    Global 5-minute mission timer — RTL if exceeded at any point.
...
Failsafes:
    Ctrl+C → RTL | Exception → RTL | 5-min timer → RTL
...
def main(args):
    ...
    if not args.dry_run and not args.sitl:
        ...
        desc = (
            ...
            f"  Time limit:  {MISSION_TIME_LIMIT}s (5 min)\n"
    ...
    with SafeFlight(fc, camera=cap, video_writer=vw) as sf:
        ...
        while state not in ("DONE", "ABORT"):
            ...
            # ══════════════════════════════════════════════════
                ...
                # Leg 0: face H,        fwd 3m  → arrive (0, +3)
                # Leg 1: yaw 180°,      fwd 6m  → arrive (0, -3)  [passes origin]
                # Leg 2: yaw 180°,      fwd 3m  → arrive (0,  0)  [back to origin]
                # Leg 3: yaw 90° CCW,   fwd 3m  → arrive (-3, 0)
                # Leg 4: yaw 180°,      fwd 6m  → arrive (+3, 0)  [passes origin]
                # Leg 5: yaw 180°,      fwd 3m  → arrive (0,  0)  [back to origin]

                #           (yaw_angle, yaw_direction, move_dist, label)
                cross_legs = [
                    (0,    0,  3.0, "FWD 3m to (0,+3)"),
                    (180,  1,  6.0, "FLIP+FWD 6m to (0,-3)"),
                    (180,  1,  3.0, "FLIP+FWD 3m back to origin"),
                    (90,  -1,  3.0, "CCW 90°+FWD 3m to (-3,0)"),
                    (180,  1,  6.0, "FLIP+FWD 6m to (+3,0)"),
                    (180,  1,  3.0, "FLIP+FWD 3m back to origin"),
                ]
